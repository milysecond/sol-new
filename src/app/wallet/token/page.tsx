    ...
    <WalletShell>
      ...
      {loading ? (
        ...
        <div className="text-center py-12 space-y-3">
          ...
          <a href="/token" className="text-fuchsia-400 hover:text-fuchsia-300 text-sm transition">Launch your first token</a>
        </div>
      ) : (
          ...
          {tokens.map((token) => (
            <a key={token.id} href={token.mint_address ? `/launch/${token.mint_address}` : undefined} className="flex items-center gap-3 bg-black/[0.03] dark:bg-white/[0.03] border border-black/10 dark:border-white/10 rounded-xl px-3 py-2.5 hover:border-fuchsia-400/30 transition cursor-pointer">
              {token.image_url ? (
                ...
                <div className="w-9 h-9 rounded-lg bg-fuchsia-500/20 flex items-center justify-center shrink-0"><Coins size={16} className="text-fuchsia-400" /></div>
              )}
              <div className="flex-1 min-w-0">
                ...
                {token.mint_address && (
                  <p className="text-fuchsia-400/50 text-[10px] font-mono truncate">{token.mint_address}</p>
                )}
              </div>
