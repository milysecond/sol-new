    ...
    return (
      <div className="space-y-4">
        <div className="bg-white/5 border border-white/10 rounded-2xl p-6 flex flex-col items-center space-y-4">
          ...
          <div onClick={copyLink} className="w-full bg-black/50 rounded-lg px-4 py-3 font-mono text-xs text-white/50 break-all cursor-pointer hover:text-white/70 transition">
            {payUrl}
          </div>
          <button onClick={copyLink} className="w-full bg-purple-500 hover:bg-purple-400 text-white font-semibold rounded-xl px-4 py-3 transition cursor-pointer">
            {copied ? "Copied! ✓" : "Copy link"}
          </button>
        </div>
        <button onClick={reset} className="w-full bg-white/5 border border-white/10 text-white/60 rounded-xl px-4 py-3 hover:text-white transition cursor-pointer">
          Create another →
        </button>
      </div>
    );
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex gap-2">
        {TOKENS.map((token) => (
          <button key={token} onClick={() => setSelected(token)} className={`flex-1 border rounded-xl px-4 py-2.5 text-sm transition cursor-pointer ${selected === token ? "bg-purple-500/20 border-purple-400/50 text-purple-300" : "bg-white/5 border-white/10 text-white/60 hover:text-white hover:border-white/20"}`}>
            {token}
          </button>
        ))}
      </div>
